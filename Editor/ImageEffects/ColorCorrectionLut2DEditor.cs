...
{
	[CustomEditor (typeof(ColorCorrectionLut2D))]
	class ColorCorrectionLut2DEditor : Editor
	{
		...
		public override void OnInspectorGUI () {
			...
			if (tempClutTex2D == null) {
				t = AssetDatabase.LoadMainAssetAtPath(((ColorCorrectionLut2D)target).basedOnTempTex) as Texture2D;
				if (t) tempClutTex2D = t;
			}
			
			Texture2D tex = tempClutTex2D;
			
			if (tex && (target as ColorCorrectionLut2D).basedOnTempTex != AssetDatabase.GetAssetPath(tex))
			{
				EditorGUILayout.Separator();
				if (!(target as ColorCorrectionLut2D).ValidDimensions(tex))
				{
				...
				else if (GUILayout.Button ("Convert and Apply"))
				{
					...
					(target as ColorCorrectionLut2D).Convert(tex, path);
				}
			}
			
			if ((target as ColorCorrectionLut2D).basedOnTempTex != "")
			{
				EditorGUILayout.HelpBox("Using " + (target as ColorCorrectionLut2D).basedOnTempTex, MessageType.Info);
				t = AssetDatabase.LoadMainAssetAtPath(((ColorCorrectionLut2D)target).basedOnTempTex) as Texture2D;
				if (t) {
